...
public class OVRLipSync : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            Debug.LogWarning("OVRLipSync currently only supports Windows platforms, this feature will be disabled");
            sInitialized = Result.BadVersion;
            return;
        }
        ...
        if (IsInitialized() != Result.Success)
        {
            ...
            {
                Debug.LogError("OVRLipSync initialization failed: OVRLipSync.dll library not found");
                sInitialized = Result.MissingDLL;
            }
            catch (Exception e)
            {
                Debug.LogError("OVRLipSync initialization exception: " + e.Message);
    ...
    public static Result Initialize()
    {
        if (!IsPlatformSupported())
        {
            Debug.LogWarning("OVRLipSync is not supported on current platform");
