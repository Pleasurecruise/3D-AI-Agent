...
public class XunfeiTextToSpeech : TTS
{
    ...
    public IEnumerator GetSpeech(string _text, Action<AudioClip, string> _callback)
    {
        ...
        stopwatch.Stop();
        UnityEngine.Debug.Log("Xunfei Speech Synthesis Time: " + stopwatch.Elapsed.TotalSeconds);
    }
        ...
        try
        {
            ...
            while (m_WebSocket.State == WebSocketState.Open)
            {
                ...
                if (str.EndsWith("}"))
                {
                    ...
                    {
                        Debug.LogError("Empty audio data returned");
                        m_WebSocket.Abort();
                        break;
                    }
                    //拿到音频数据
    ...
    private void PrintErrorLog(int status)
    {
        if (status == 10005)
        {
            Debug.LogError("AppID authorization failed");
            return;
        }
        if (status == 10006)
        {
            Debug.LogError("Missing required parameters");
            return;
        }
        if (status == 10007)
        {
            Debug.LogError("Invalid parameter value");
            return;
        }
        if (status == 10010)
        {
            Debug.LogError("Insufficient engine authorization");
            return;
        }
        if (status == 10109)
        {
            Debug.LogError("Illegal text length");
            return;
        }
        if (status == 10019)
        {
        ...
        if (status == 10101)
        {
            Debug.LogError("Engine session has ended");
            return;
        }
        if (status == 10313)
        {
            Debug.LogError("AppID cannot be empty");
            return;
        }
        if (status == 10317)
        {
            Debug.LogError("Illegal version");
            return;
        }
        if (status == 11200)
        {
        ...
        if (status == 11201)
        {
            Debug.LogError("Daily flow control exceeded");
            return;
        }
        if (status == 10160)
        {
            Debug.LogError("Illegal request data format");
            return;
        }
        if (status == 10161)
        {
            Debug.LogError("Base64 decoding failed");
            return;
        }
        if (status == 10163)
        {
        ...
        if (status == 10200)
        {
            Debug.LogError("Data read timeout");
            return;
        }
        if (status == 10222)
        {
