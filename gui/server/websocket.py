...
class MyServer:
    ...
    async def __consumer_handler(self, websocket, path):
        ...
        try:
            ...
            logger.printInfo(1, "User" if username is None else username, f"WebSocket 连接关闭: {e}")
    ...
    async def send_message_with_timeout(self, client, message, username, timeout=3):
        try:
            ...
            logger.printInfo(1, "User" if username is None else username, f"发送消息超时: 用户名 {username}")
    ...
    async def __handler(self, websocket, path):
        self.isConnect = True
        logger.log(1, "websocket连接上:{}".format(self.__port))
        ...
        # 从客户端列表中移除已断开的连接
        await self.remove_client(websocket)
        logger.log(1, "websocket连接断开:{}".format(unique_id))
    ...
    def __connect(self):
        ...
        if self.__server:
            logger.log(1, 'server already exist')
            return
    ...
    def stop_server(self):
        ...
        self.__server = None
        self.__clients = []
        logger.log(1, "WebSocket server stopped.")
